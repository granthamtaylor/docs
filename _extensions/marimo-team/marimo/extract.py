...
def marimo_table_to_static_html(markup: str) -> str | None:
    ...
    columns = [
        ...
        and field_type
        and isinstance(field_type[0], str)
    ]
        ...
        if show_output and static_output and not mime_sensitive:
            ...
            if static_html is not None:
                return {
                    "type": "html",
                    "value": (
                        '<div class="qmo-static-cell-output">'
                        f"{static_html}"
                        "</div>"
                    ),
                    **render_options,
...
def build_export_with_mime_context(
    ...
    def tree_to_pandoc_export(root: Element) -> SafeWrap:  # type: ignore[valid-type]
        ...
        needs_runtime = any(
            "<marimo-" in str(output.get("value", "")) for output in outputs
        )
...
if __name__ == "__main__":
    ...
    static_output = (
        ...
        if len(sys.argv) == 5
        else bool(default_config["marimo-static-output"])
    )
